...
@click.option('--subset_ratio', type=click.FloatRange(min=0., max=1.), default=1.0, help="By default the whole data will be written, Choose a values < 1 to write a subset.")
@click.option('--seed', type=click.INT, default=42, help="Random seed is only considered when subset_ratio is less than 1.0.")
...
def prepare_data(dataset, subset_ratio, seed, save_loaded_data_folder, save_new_data_folder):
	Path(save_loaded_data_folder).mkdir(parents=True, exist_ok=True)
	Path(save_new_data_folder).mkdir(parents=True, exist_ok=True)

	trainset, testset, train_labels, test_labels = dispatcher[dataset](save_loaded_data_folder)

	train_ids = np.arange(len(train_labels))
	test_ids = np.arange(len(test_labels))
	if subset_ratio is not None:
		trainset, _, train_ids = subset_selection(dataset=trainset, labels=train_labels, ratio=subset_ratio, random_seed=seed)
		testset, _, test_ids = subset_selection(dataset=testset, labels=test_labels, ratio=subset_ratio, random_seed=seed)

	trainset = convert_data_to_tensor_dataset(trainset)
	testset = convert_data_to_tensor_dataset(testset)

	inner_folder_name = f'subset_id{seed}_r{subset_ratio}' if subset_ratio < 1 else f'full'
