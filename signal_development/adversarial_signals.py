...
def load_data(data_name, model_name, inf_fn_name, subset_id, attack, device):
	...
	model_info_fp = Path('results', model_name, data_name, 'clean', 'ckpts', 'info.json')
...
def plot_si_interval_accuract(si_acc_info, savedir=None, fname=None):
	si_info_accs = pd.DataFrame(si_acc_info)
	si_info_accs = si_info_accs.groupby(by=0).mean()
	si_info_accs.index.name = None
	si_info_accs.T.plot(style='.-')
	plt.xticks([])
	plt.ylabel('Test Accuracy')
	plt.xlabel('Increasing Self Influence')
	plt.title('Test Accuracy vs Self Influence on Clean Test Data')
	Path(savedir).mkdir(parents=True, exist_ok=True)
	if savedir is not None:
		fname = fname if fname is not None else 'sig_acc.png'
		plt.savefig(Path(savedir, fname), dpi=300)
	plt.show()
...
if __name__ == '__main__':
	...
	for d in data:
		for a in attack_choices:
			...
			df_split = np.array_split(df.drop(index=holder.adv_ids).sort_values(by='SI'), 10)
			accs = []
			for sdf in df_split:
				accs.append(accuracy_score(sdf['y_obs'], sdf['y_pred']))
			si_acc_info.append([d, *accs])
	...
	plot_si_interval_accuract(si_acc_info, Path('figures','signals_acc_misclass'), 'si.png')
